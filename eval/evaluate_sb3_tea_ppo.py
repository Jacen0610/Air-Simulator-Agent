...
def main():
    ...
    while episodes_completed < EVAL_EPISODES:
        ...
        if previous_is_busy == 1 and current_is_busy == 0:
            if not waiting_for_send_after_busy:
                waiting_for_send_after_busy = True
                steps_since_idle = 0 # 开始计数
        
        # 如果处于等待状态，则计数器增加
        if waiting_for_send_after_busy:
            steps_since_idle += 1
        ...
        # [新增] 如果在等待时发送，则记录延迟
        if waiting_for_send_after_busy and action[0] == 1:
            episode_send_delays.append(steps_since_idle)
            model.logger.record("trace/send_delay_after_idle", steps_since_idle)
            # 重置状态机，等待下一次 1->0 跳变
            waiting_for_send_after_busy = False
            steps_since_idle = 0
        ...
        # 更新上一步的状态
        previous_is_busy = current_is_busy
        ...
        if 'episode' in info[0]:
            ...
            # [新增] 计算平均延迟
            avg_delay = np.mean(episode_send_delays) if episode_send_delays else 0
            
            # [新增] 计算并打印 SPS
            elapsed_time = time.time() - start_time
            avg_sps = total_steps / elapsed_time if elapsed_time > 0 else 0
            ...
            print(f"评估 Episode {episodes_completed}/{EVAL_EPISODES} | "
                  ...
                  f"Avg SPS: {avg_sps:.2f} | "
                  f"Avg Delay: {avg_delay:.2f} steps")
            ...
            model.logger.record("eval/sps", avg_sps) # [新增] 记录 SPS
            model.logger.record("eval/mean_send_delay", avg_delay) # [新增] 记录平均延迟
            model.logger.dump(step=total_steps)
            
            # 重置统计列表
            episode_values = []
            episode_entropies = []
            episode_send_delays = [] # [新增] 重置延迟列表
