...
def main():
    ...
    """
    # --- 解析命令行参数 ---
    parser = argparse.ArgumentParser(description='Evaluate SB3 Transformer PPO Agent')
    ...
    MODEL_PATH = os.path.join(project_root, "SB3/models/sb3_transformer_ppo_final.zip")
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print("请先运行 train_sb3_attention_ppo.py 脚本来训练并保存一个模型。")
        return
    if not os.path.exists(STATS_PATH):
        print(f"错误：找不到环境统计数据文件 '{STATS_PATH}'。")
        print("请确保 train_sb3_attention_ppo.py 脚本已运行并保存了 VecNormalize 统计数据。")
        return
    ...
    raw_env = GymEnv(grpc_server_address=grpc_address, sequence_length=SEQUENCE_LENGTH)
    # 2. 使用 Monitor 包装
    ...
    PLOT_FILENAME = os.path.join(PLOT_DIR, f"sb3_transformer_ppo_evaluation_rewards_{timestamp}.png")
    plot_evaluation_rewards(
        eval_rewards,
        f"SB3 Transformer PPO Model Evaluation Rewards (Avg: {np.mean(eval_rewards):.2f})",
        PLOT_FILENAME
    )
