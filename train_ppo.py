...
class PPOConfig:
    ...
    def __init__(self):
        ...
        # 训练过程参数
        self.max_episodes = 20         # 总共训练的回合数
        self.update_timestep = 20000      # 每隔多少步更新一次网络

        # PPO 核心超参数
        self.lr = 0.0003                 # 学习率
        self.gamma = 0.99                # 折扣因子
        self.K_epochs = 4                # 更新策略的 epoch 数
...
def main():
    ...
    if config.resume_from_episode > 0:
        ...
        for agent_id, agent in agents.items():
            checkpoint_path = f"{config.checkpoint_dir}/ppo_agent_{agent_id}_episode_{config.resume_from_episode}.pth"
    ...
    for i_episode in range(start_episode, config.max_episodes + 1):
        ...
        if i_episode % config.save_every_episodes == 0:
            ...
            for agent_id, agent in agents.items():
                checkpoint_path = f"{config.checkpoint_dir}/ppo_agent_{agent_id}_episode_{i_episode}.pth"
