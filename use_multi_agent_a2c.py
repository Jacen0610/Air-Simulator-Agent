...
def find_latest_episode(checkpoint_dir):
    ...
    max_episode = 0
    # 正则表达式匹配 "a2c_agent_..._episode_(\\d+).pth"
    pattern = re.compile(r'a2c_agent_.*_episode_(\d+)\.pth')
...
def main():
    ...
    # 在推理时，学习率和折扣因子不是必需的，但为了与 Agent 类兼容，我们传入默认值
    agents = {agent_id: A2CAgent(config.state_dim, config.action_dim, 0.0003, 0.99) for agent_id in agent_ids}

    # --- 加载模型权重 ---
    all_agents_loaded = True
    for agent_id, agent in agents.items():
        checkpoint_path = f"{config.checkpoint_dir}/a2c_agent_{agent_id}_episode_{load_episode}.pth"
